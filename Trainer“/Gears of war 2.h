...
static void __declspec(naked) CodeCave_hp(void)
{
	...
	if(addr < 0x70000000)
	{
		if(*(DWORD*)(addr + 4) == 0x582 || *(DWORD*)(addr + 4) == 0x258) // player
		{
			if(InfHealth = TRUE)
			{
...
{
	Prompt = XShowMessageBoxUI(0, L"More at T3furyGaming.co.uk", L"Gears of War 2 + 4\r\n~~~~~~~~~~~~~~~~~~~~~~~~~\r\nView Available Codes: DPAD-DOWN(x4)\r\nActivate All Codes: DPAD-UP(x4)\r\n", ARRAYSIZE(g_strButtons), g_strButtons, 0, XMB_PASSCODEMODE, &g_mb_result, &g_xol);
}
...
VOID SearchAddr()
{
	...
	DWORD addr;
	DWORD buf[] = {0x7D2A5850, 0x913D0000};
