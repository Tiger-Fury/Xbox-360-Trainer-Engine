...
static void Statics(){
	while (1){
		...
		if (InfHealth == TRUE)
		{
			SetWORD(0x8250CCA2, 0x1fff);
		}  

		if (InfSpecial == TRUE)
		{
			SetWORD(0x8250CCa8, 0x0ca0);
...
{
	Prompt = XShowMessageBoxUI(0, L"More at T3furyGaming.co.uk", L"Dynasty Warriors 5 Empires + 2\r\n~~~~~~~~~~~~~~~~~~~~~~~~~\r\nView Available Codes: DPAD-DOWN(x4)\r\nActivate All Codes: DPAD-UP(x4)\r\n", ARRAYSIZE(g_strButtons), g_strButtons, 0, XMB_PASSCODEMODE, &g_mb_result, &g_xol);
...
VOID SearchAddr()
{
	hNotify(L"Dynasty Warriors 5 Empires");
